...
void DoNamingScreen(u8 templateNum, u8 *destBuffer, u16 monSpecies, u8 monForm, u16 monGender, u32 monPersonality, MainCallback returnCallback)
{
...
static void DisplaySentToPCMessage(void)
{
    ...
    gTextFlags.canABSpeedUpPrint = TRUE;
    AddTextPrinterParameterized2(0, FONT_OPTION, gStringVar4, GetPlayerTextSpeedDelay(), 0, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
...
static void NamingScreen_CreatePlayerIcon(void)
{
    ...
    spriteId = CreateObjectGraphicsSprite(rivalGfxId, SpriteCallbackDummy, 56, 37, 0, FALSE);
    gSprites[spriteId].oam.priority = 3;
...
static void NamingScreen_CreateMonIcon(void)
{
    ...
    LoadMonIconPalettes();
    spriteId = CreateMonIcon(sNamingScreen->monSpecies, sNamingScreen->monForm, SpriteCallbackDummy, 56, 40, 0, sNamingScreen->monPersonality, 1);
    gSprites[spriteId].oam.priority = 3;
}
...
static void NamingScreen_CreateWaldaDadIcon(void)
{
    ...
    spriteId = CreateObjectGraphicsSprite(OBJ_EVENT_GFX_MAN_1, SpriteCallbackDummy, 56, 37, 0, FALSE);
    gSprites[spriteId].oam.priority = 3;
    StartSpriteAnim(&gSprites[spriteId], ANIM_STD_GO_SOUTH);
}
...
static void DrawNormalTextEntryBox(void)
{
    ...
    AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX], FONT_OPTION, sNamingScreen->template->title, 8, 1, 0, 0);
...
static void DrawMonTextEntryBox(void)
{
    ...
    FillWindowPixelBuffer(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX], PIXEL_FILL(1));
    AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX], FONT_OPTION, buffer, 8, 1, 0, 0);
    PutWindowTilemap(sNamingScreen->windows[WIN_TEXT_ENTRY_BOX]);
}
...
static void DrawGenderIcon(void)
{
    ...
    if (sNamingScreen->monGender != MON_GENDERLESS)
    {
        ...
        AddTextPrinterParameterized3(sNamingScreen->windows[WIN_TEXT_ENTRY], FONT_OPTION, (POKEMON_NAME_LENGTH * 4) + 64, 1, sGenderColors[isFemale], TEXT_SKIP_DRAW, text);
    }
}
...
static void DrawTextEntry(void)
{
    ...
    for (i = 0; i < maxChars; i++)
    {
        ...
        AddTextPrinterParameterized(sNamingScreen->windows[WIN_TEXT_ENTRY], FONT_OPTION, temp, i * 8 + x + extraWidth, 1, TEXT_SKIP_DRAW, NULL);
    }
...
static void PrintKeyboardKeys(u8 window, u8 page)
{
    ...
    for (i = 0; i < KBROW_COUNT; i++)
        AddTextPrinterParameterized3(window, FONT_OPTION, 0, i * 16 + 1, sKeyboardTextColors[page], 0, sNamingScreenKeyboardText[page][i]);
