...
static void ErrorMsg_MoveCloserToPartner(void)
{
    ...
    AddTextPrinterParameterized3(WIN_LINK_ERROR_TOP, FONT_FRLGE, 2, 6, sTextColors, 0, gText_CommErrorEllipsis);
    AddTextPrinterParameterized3(WIN_LINK_ERROR_BOTTOM, FONT_FRLGE, 2, 1, sTextColors, 0, gText_MoveCloserToLinkPartner);
...
static void ErrorMsg_CheckConnections(void)
{
    ...
    FillWindowPixelBuffer(WIN_LINK_ERROR_BOTTOM, PIXEL_FILL(0));
    AddTextPrinterParameterized3(WIN_LINK_ERROR_MID, FONT_FRLGE, 2, 0, sTextColors, 0, gText_CommErrorCheckConnections);
    ...
    switch (gMain.state)
    {
        ...
        case 130:
            if (gWirelessCommType == 2)
                AddTextPrinterParameterized3(WIN_LINK_ERROR_TOP, FONT_FRLGE, 2, 20, sTextColors, 0, gText_ABtnTitleScreen);
            else if (gWirelessCommType == 1)
                AddTextPrinterParameterized3(WIN_LINK_ERROR_TOP, FONT_FRLGE, 2, 20, sTextColors, 0, gText_ABtnRegistrationCounter);
            break;
    }
    if (gMain.state == 160)
    {
