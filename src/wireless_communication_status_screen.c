...
static void PrintHeaderTexts(void)
{
    ...
    // Print title
    WCSS_AddTextPrinterParameterized(0, FONT_OPTION, sHeaderTexts[0], GetStringCenterAlignXOffset(FONT_OPTION, sHeaderTexts[0], 0xC0), 6, COLORMODE_GREEN);

    // Print label for each group (excluding total)
    for (i = 0; i < NUM_GROUPTYPES - 1; i++)
        WCSS_AddTextPrinterParameterized(1, FONT_OPTION, sHeaderTexts[i + 1], 0, 30 * i + 8, COLORMODE_WHITE_LGRAY);

    // Print label for total
    WCSS_AddTextPrinterParameterized(1, FONT_OPTION, sHeaderTexts[i + 1], 0, 30 * i + 8, COLORMODE_RED);
        ...
        if (UpdateCommunicationCounts(sStatusScreen->groupCounts, sStatusScreen->prevGroupCounts, sStatusScreen->activities, sStatusScreen->rfuTaskId))
        {
            ...
            for (i = 0; i < NUM_GROUPTYPES; i++)
            {
                ...
                if (i != GROUPTYPE_TOTAL)
                    WCSS_AddTextPrinterParameterized(2, FONT_OPTION, gStringVar4, 12, 30 * i + 8, COLORMODE_WHITE_LGRAY);
                else
                    WCSS_AddTextPrinterParameterized(2, FONT_OPTION, gStringVar4, 12, 98, COLORMODE_RED);
            }
            PutWindowTilemap(2);
