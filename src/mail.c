...
static bool8 MailReadBuildGraphics(void)
{
    ...
    switch (gMain.state)
    {
        ...
        case 17:
            ...
            {
            case ICON_TYPE_BEAD:
                LoadMonIconPalette(icon, FORM_NONE);
                sMailRead->monIconSpriteId = CreateMonIconNoPersonality(icon, FORM_NONE, SpriteCallbackDummy, 96, 128, 0, FALSE);
                break;
            case ICON_TYPE_DREAM:
                LoadMonIconPalette(icon, FORM_NONE);
                sMailRead->monIconSpriteId = CreateMonIconNoPersonality(icon, FORM_NONE, SpriteCallbackDummy, 40, 128, 0, FALSE);
                break;
            }
            break;
        case 18:
...
static void PrintMailText(void)
{
    ...
    for (i = 0; i < sMailRead->layout->numLines; i ++)
    {
        ...
        AddTextPrinterParameterized3(0, FONT_OPTION, sMailRead->layout->lines[i].xOffset + sMailRead->layout->wordsXPos, y + sMailRead->layout->wordsYPos, sTextColors, 0, sMailRead->message[i]);
        y += sMailRead->layout->lines[i].height;
    }
    bufptr = StringCopy(signature, gText_FromSpace);
    StringCopy(bufptr, sMailRead->playerName);
    box_x = GetStringCenterAlignXOffset(FONT_OPTION, signature, sMailRead->signatureWidth) + 104;
    box_y = sMailRead->layout->signatureYPos + 88;
    AddTextPrinterParameterized3(0, FONT_OPTION, box_x, box_y, sTextColors, 0, signature);
...
static void CB2_ExitMailReadFreeVars(void)
{
    if (!UpdatePaletteFade())
    {
        ...
        case ICON_TYPE_BEAD:
        case ICON_TYPE_DREAM:
            FreeMonIconPalette(GetIconSpeciesNoPersonality(sMailRead->mail->species), FORM_NONE);
