    ...
    case 3:
        {
            ...
            PrintInfoScreenText(string, GetStringCenterAlignXOffset(FONT_OPTION, string, DISPLAY_WIDTH), 121);
            gMain.state++;
        }
        break;
    case 4:
...
static void Task_ExitCaughtMonPage(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        form = gTasks[taskId].tForm;
        paletteNum = gSprites[gTasks[taskId].tMonSpriteId].oam.paletteNum;
        lzPaletteData = GetMonSpritePalFromSpeciesAndPersonality(species, form, otId, personality);
...
static void PrintMonInfo(u32 num, u32 value, u32 owned, u32 newEntry)
{
    ...
    if (newEntry)
        PrintInfoScreenText(gText_PokedexRegistration, GetStringCenterAlignXOffset(FONT_OPTION, gText_PokedexRegistration, DISPLAY_WIDTH), 0);
    if (value == 0)
    ...
    PrintInfoScreenText(description, GetStringCenterAlignXOffset(FONT_OPTION, description, DISPLAY_WIDTH), 95);
}
...
u16 CreateMonSpriteFromNationalDexNumber(u16 nationalNum, s16 x, s16 y, u16 paletteSlot)
{
    ...
    return CreateMonPicSprite_HandleDeoxys(nationalNum, FORM_NONE, SHINY_ODDS, GetPokedexMonPersonality(nationalNum), TRUE, x, y, paletteSlot, TAG_NONE);
}
