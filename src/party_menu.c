...
static void CreatePartyMonIconSpriteParameterized(u16, u8, u32, struct PartyMenuBox *, u8, u32);
...
static void CreatePartyMonSprites(u8 slot)
{
    ...
    if (gPartyMenu.menuType == PARTY_MENU_TYPE_MULTI_SHOWCASE && slot >= MULTI_PARTY_SIZE)
    {
        ...
        if (gMultiPartnerParty[actualSlot].species != SPECIES_NONE)
        {
            CreatePartyMonIconSpriteParameterized(gMultiPartnerParty[actualSlot].species, gMultiPartnerParty[actualSlot].form, gMultiPartnerParty[actualSlot].personality, &sPartyMenuBoxes[slot], 0, FALSE);
...
static u8 CanMonLearnTMTutor(struct Pokemon *mon, u16 item, u8 tutor)
{
    ...
    {
		if (!CanLearnTutorMove(GetMonData(mon, MON_DATA_SPECIES), GetMonData(mon, MON_DATA_FORM), tutor))
		{
			return CANNOT_LEARN_MOVE;
		}
		else
...
bool8 CanLearnTutorMove(u16 species, u8 form, u8 tutor)
{
    if (IsFormValid(species, form) && gSpeciesInfo[GetFormID(species, form)].altLearnset)
    {
        if (sTutorLearnsets[GetFormID(species, form)] & ((u64)1 << (u64)tutor))
            return TRUE;
        else
            return FALSE;
    }
    ...
    {
        if (sTutorLearnsets[species] & ((u64)1 << (u64)tutor))
            return TRUE;
        else
            return FALSE;
    }
...
static void DisplayPartyPokemonDescriptionText(u8 stringID, struct PartyMenuBox *menuBox, u8 c)
{
    ...
    if (c != 2)
        AddTextPrinterParameterized3(menuBox->windowId, FONT_OPTION, menuBox->infoRects->descTextLeft, menuBox->infoRects->descTextTop, sFontColorTable[0], 0, sDescriptionStringTable[stringID]);
}
...
void DisplayPartyMenuStdMessage(u32 stringId)
{
    ...
    if (stringId != PARTY_MSG_NONE)
    {
        ...
        AddTextPrinterParameterized(*windowPtr, FONT_OPTION, gStringVar4, 0, 1, 0, 0);
        ScheduleBgCopyTilemapToVram(2);
    }
}
...
static u8 DisplaySelectionWindow(u8 windowType)
{
    ...
    cursorDimension = GetMenuCursorDimensionByFont(FONT_OPTION, 0);
    letterSpacing = GetFontAttribute(FONT_OPTION, FONTATTR_LETTER_SPACING);

    for (i = 0; i < sPartyMenuInternal->numActions; i++)
    {
        u8 fontColorsId = (sPartyMenuInternal->actions[i] >= MENU_FIELD_MOVES) ? 4 : 3;
        AddTextPrinterParameterized4(sPartyMenuInternal->windowId[0], FONT_OPTION, cursorDimension, (i * 16) + 1, letterSpacing, 0, sFontColorTable[fontColorsId], 0, sCursorOptions[sPartyMenuInternal->actions[i]].text);
    }
...
static void PrintMessage(const u8 *text)
{
    ...
    AddTextPrinterParameterized2(WIN_MSG, FONT_OPTION, text, GetPlayerTextSpeedDelay(), 0, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        {
            if (GetMonData(&mons[slotId], i + MON_DATA_MOVE1) == sFieldMoves[j] && j != 5)
            {
	...
	if (sPartyMenuInternal->numActions < 5 && CanMonLearnTMHM(&mons[slotId], 51))
...
static void CreatePartyMonIconSprite(struct Pokemon *mon, struct PartyMenuBox *menuBox, u32 slot)
{
    ...
    CreatePartyMonIconSpriteParameterized(species2, GetMonData(mon, MON_DATA_FORM), GetMonData(mon, MON_DATA_PERSONALITY), menuBox, 1, handleDeoxys);
    UpdatePartyMonHPBar(menuBox->monSpriteId, mon);
}

static void CreatePartyMonIconSpriteParameterized(u16 species, u8 form, u32 pid, struct PartyMenuBox *menuBox, u8 priority, bool32 handleDeoxys)
{
    if (species != SPECIES_NONE)
    {
        menuBox->monSpriteId = CreateMonIcon(species, form, SpriteCB_MonIcon, menuBox->spriteCoords[0], menuBox->spriteCoords[1], 4, pid, handleDeoxys);
...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    u8 candyType = ItemId_GetSecondaryId(*itemPtr);
    ...
    if (cannotUseEffect)
    {
        ...
        if (targetSpecies != SPECIES_NONE && candyType == CANDY_RARE)
        {
        ...
        if (sFinalLevel > sInitialLevel)
        {
            ...
            else
            {
                switch (candyType)
                {
                    case CANDY_XS:
                    default:
                        ConvertIntToDecimalStringN(gStringVar2, CANDY_XS_YIELD, STR_CONV_MODE_LEFT_ALIGN, 6);
                        break;
                    case CANDY_S:
                        ConvertIntToDecimalStringN(gStringVar2, CANDY_S_YIELD, STR_CONV_MODE_LEFT_ALIGN, 6);
                        break;
                    case CANDY_M:
                        ConvertIntToDecimalStringN(gStringVar2, CANDY_M_YIELD, STR_CONV_MODE_LEFT_ALIGN, 6);
                        break;
                    case CANDY_L:
                        ConvertIntToDecimalStringN(gStringVar2, CANDY_L_YIELD, STR_CONV_MODE_LEFT_ALIGN, 6);
                        break;
                    case CANDY_XL:
                        ConvertIntToDecimalStringN(gStringVar2, CANDY_XL_YIELD, STR_CONV_MODE_LEFT_ALIGN, 6);
                        break;
                }
...
static void PartyMenuTryEvolution(u8 taskId)
{
    ...
    {
        targetSpecies = GetEvolutionTargetSpecies(mon, EVO_MODE_TRADE, GetMonData(mon, MON_DATA_HELD_ITEM, NULL));
