...
static struct Sprite *CreateMonIconSprite(u16, u8, u32, s16, s16, u8, u8);
...
static void LoadDisplayMonGfx(u16, u8, u32);
...
void DrawTextWindowAndBufferTiles(const u8 *string, void *dst, u8 zero1, u8 zero2, s32 bytesToBuffer)
{
    ...
    AddTextPrinterParameterized4(windowId, FONT_OPTION, 0, 1, 0, 0, txtColor, TEXT_SKIP_DRAW, string);
...
static void UNUSED UnusedDrawTextWindow(const u8 *string, void *dst, u16 offset, u8 bgColor, u8 fgColor, u8 shadowColor)
{
    ...
    AddTextPrinterParameterized4(windowId, FONT_OPTION, 0, 2, 0, 0, txtColor, TEXT_SKIP_DRAW, string);
...
static void Task_PCMainMenu(u8 taskId)
{
    ...
    case STATE_LOAD:
        ...
        AddTextPrinterParameterized2(0, FONT_OPTION, sMainMenuTexts[task->tSelectedOption].desc, TEXT_SKIP_DRAW, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
    ...
    case STATE_HANDLE_INPUT:
        ...
        case MENU_NOTHING_CHOSEN:
            ...
            if (task->tSelectedOption != task->tNextOption)
            {
                ...
                FillWindowPixelBuffer(0, PIXEL_FILL(1));
                AddTextPrinterParameterized2(0, FONT_OPTION, sMainMenuTexts[task->tSelectedOption].desc, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
            }
            break;
        case MENU_B_PRESSED:
        case OPTION_EXIT:
        ...
        default:
            if (task->tInput == OPTION_WITHDRAW && CountPartyMons() == PARTY_SIZE)
            {
                // Can't withdraw
                FillWindowPixelBuffer(0, PIXEL_FILL(1));
                AddTextPrinterParameterized2(0, FONT_OPTION, gText_PartyFull, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
            ...
            else if (task->tInput == OPTION_DEPOSIT && CountPartyMons() == 1)
            {
                // Can't deposit
                FillWindowPixelBuffer(0, PIXEL_FILL(1));
                AddTextPrinterParameterized2(0, FONT_OPTION, gText_JustOnePkmn, 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
                task->tState = STATE_ERROR_MSG;
            }
            else
            {
...
static void ChooseBoxMenu_PrintInfo(void)
{
    ...
    // Print box name
    center = GetStringCenterAlignXOffset(FONT_OPTION, boxName, 64);
    AddTextPrinterParameterized3(windowId, FONT_OPTION, center, 1, sChooseBoxMenu_TextColors, TEXT_SKIP_DRAW, boxName);
    ...
    center = GetStringCenterAlignXOffset(FONT_OPTION, numBoxMonsText, 64);
    AddTextPrinterParameterized3(windowId, FONT_OPTION, center, 17, sChooseBoxMenu_TextColors, TEXT_SKIP_DRAW, numBoxMonsText);
...
static void RefreshDisplayMonData(void)
{
    LoadDisplayMonGfx(sStorage->displayMonSpecies, sStorage->displayMonForm, sStorage->displayMonPersonality);
    PrintDisplayMonInfo();
...
static void LoadDisplayMonGfx(u16 species, u8 form, u32 pid)
{
	...
	if (species == SPECIES_EGG)
	{
		...
		LoadSpecialPokePic(&gMonFrontPicTable[species], sStorage->tileBuffer, species, form, pid, TRUE);
    ...
    else if (species != SPECIES_NONE)
    {
		...
		UniquePalette(sStorage->displayMonPalOffset, species, form, pid, (bool8)sStorage->filler2[2]);
...
static void PrintDisplayMonInfo(void)
{
    ...
    if (sStorage->boxOption != OPTION_MOVE_ITEMS)
    {
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FONT_OPTION, sStorage->displayMonNameText, 6, 0, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FONT_FRLGE, sStorage->displayMonSpeciesName, 6, 15, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FONT_FRLGE, sStorage->displayMonGenderLvlText, 10, 29, TEXT_SKIP_DRAW, NULL);
    ...
    else
    {
        ...
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FONT_OPTION, sStorage->displayMonNameText, 6, 13, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FONT_FRLGE, sStorage->displayMonSpeciesName, 6, 28, TEXT_SKIP_DRAW, NULL);
        AddTextPrinterParameterized(WIN_DISPLAY_INFO, FONT_FRLGE, sStorage->displayMonGenderLvlText, 10, 42, TEXT_SKIP_DRAW, NULL);
    }
...
static void PrintMessage(u8 id)
{
    ...
    AddTextPrinterParameterized(WIN_MESSAGE, FONT_OPTION, sStorage->messageText, 0, 1, TEXT_SKIP_DRAW, NULL);
...
static void CreateMovingMonIcon(void)
{
    ...
    u16 form = GetMonData(&sStorage->movingMon, MON_DATA_FORM);
    u8 priority = GetMonIconPriorityByCursorPos();

    sStorage->movingMonSprite = CreateMonIconSprite(species, form, personality, 0, 0, priority, 7);
    ...
    for (i = 0; i < IN_BOX_ROWS; i++)
    {
        for (j = 0; j < IN_BOX_COLUMNS; j++)
        {
            ...
            if (species != SPECIES_NONE)
            {
                ...
                sStorage->boxMonsSprites[count] = CreateMonIconSprite(species, form, personality, 8 * (3 * j) + 100, 8 * (3 * i) + 44, 2, 19 - j);
            }
            else
            {
...
static void CreateBoxMonIconAtPos(u8 boxPosition)
{
    ...
    if (species != SPECIES_NONE)
    {
        ...
        sStorage->boxMonsSprites[boxPosition] = CreateMonIconSprite(species, form, personality, x, y, 2, 19 - (boxPosition % IN_BOX_COLUMNS));
        if (sStorage->boxOption == OPTION_MOVE_ITEMS)
            sStorage->boxMonsSprites[boxPosition]->oam.objMode = ST_OAM_OBJ_BLEND;
    }
}
    ...
    for (i = 0; i < IN_BOX_ROWS; i++)
    {
        for (j = 0; j < IN_BOX_COLUMNS; j++)
        {
            ...
            if (sStorage->boxSpecies[boxPosition] != SPECIES_NONE)
                ...
                sStorage->boxForms[boxPosition] = GetBoxMonDataAt(boxId, boxPosition, MON_DATA_FORM);
            boxPosition++;
        }
    }
...
static void CreatePartyMonsSprites(bool8 visible)
{
    ...
    u16 form = GetMonData(&gPlayerParty[0], MON_DATA_FORM);
    u32 personality = GetMonData(&gPlayerParty[0], MON_DATA_PERSONALITY);

    sStorage->partySprites[0] = CreateMonIconSprite(species, form, personality, 104, 64, 1, 12);
    count = 1;
    for (i = 1; i < PARTY_SIZE; i++)
    {
        ...
        if (species != SPECIES_NONE)
        {
            ...
            sStorage->partySprites[i] = CreateMonIconSprite(species, form, personality, 152,  8 * (3 * (i - 1)) + 16, 1, 12);
            count++;
        }
        else
        {
...
static u16 TryLoadMonIconTiles(u16 species, u8 form)
{
    u16 i, offset;
    ...
    offset = 16 * i;
    CpuCopy32(GetMonIconTiles(species, form, TRUE), (void *)(OBJ_VRAM0) + offset * TILE_SIZE_4BPP, 0x200);
...
static struct Sprite *CreateMonIconSprite(u16 species, u8 form, u32 personality, s16 x, s16 y, u8 oamPriority, u8 subpriority)
{
    u16 tileNum;
    u8 spriteId;
    ...
    if (IsFormValid(species, form))
        template.paletteTag = PALTAG_MON_ICON_0 + gMonIconPaletteIndices[GetFormID(species, form)];
    else
        template.paletteTag = PALTAG_MON_ICON_0 + gMonIconPaletteIndices[species];

    tileNum = TryLoadMonIconTiles(species, form);
    if (tileNum == 0xFFFF)
        return NULL;
...
{
    return DISPLAY_WIDTH - 64 - GetStringWidth(FONT_OPTION, string, 0) / 2;
}
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    else if (mode == MODE_BOX)
    {
        ...
        if (sStorage->displayMonSpecies != SPECIES_NONE)
        {
            ...
            sStorage->displayMonPalette = GetMonSpritePalFromSpeciesAndPersonality(sStorage->displayMonSpecies, sStorage->displayMonForm, otId, sStorage->displayMonPersonality);
...
static void MultiMove_SetIconToBg(u8 x, u8 y)
{
    ...
    if (species != SPECIES_NONE)
    {
        const u8 *iconGfx = GetMonIconPtr(species, form, personality, 1);
...
static void PrintItemDescription(void)
{
    ...
    AddTextPrinterParameterized5(WIN_ITEM_DESC, FONT_OPTION, description, 4, 0, 0, NULL, 0, 1);
}
...
void CreateBoxMonAt(u8 boxId, u8 boxPosition, u16 species, u8 form, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 personality, u8 otIDType, u32 otID)
{
    if (boxId < TOTAL_BOXES_COUNT && boxPosition < IN_BOX_COUNT)
    {
