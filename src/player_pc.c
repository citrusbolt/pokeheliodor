...
void NewGameInitPCItems(void)
{
    ...
    while (TRUE)
    {
        if (sNewGamePCItems[i][0] == ITEM_NONE || sNewGamePCItems[i][1] == 0)
            break;
        if (AddPCItem(sNewGamePCItems[i][0], sNewGamePCItems[i][1], NULL) != TRUE)
            break;
        i++;
    }
}
...
static void ItemStorageMenuPrint(const u8 *textPtr)
{
    ...
    AddTextPrinterParameterized(0, FONT_OPTION, textPtr, 0, 1, 0, 0);
}
...
static void Mailbox_DrawMailboxMenu(u8 taskId)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_OPTION, gText_Mailbox, GetStringCenterAlignXOffset(FONT_OPTION, gText_Mailbox, 0x40), 1, 0, NULL);
    ScheduleBgCopyTilemapToVram(0);
...
static void ItemStorage_PrintDescription(s32 id)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_OPTION, description, 0, 1, 0, NULL);
}
...
static void ItemStorage_DrawSwapArrow(u8 y, u8 b, u8 speed)
{
    ...
    if (b == 0xFF)
        FillWindowPixelRect(windowId, PIXEL_FILL(1), 0, y, GetMenuCursorDimensionByFont(FONT_OPTION, 0), GetMenuCursorDimensionByFont(FONT_OPTION, 1));
    else
        AddTextPrinterParameterized4(windowId, FONT_OPTION, 0, y, 0, 0, sSwapArrowTextColors, speed, gText_SelectorArrow2);
}
...
static void ItemStorage_CreateListMenu(u8 taskId)
{
    ...
    x = GetStringCenterAlignXOffset(FONT_OPTION, text, 104);
    AddTextPrinterParameterized(sItemStorageMenu->windowIds[ITEMPC_WIN_TITLE], FONT_OPTION, text, x, 1, 0, NULL);
...
static void ItemStorage_PrintMessage(const u8 *string)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_OPTION, gStringVar4, 0, 1, 0, NULL);
}
...
static void ItemStorage_PrintItemQuantity(u8 windowId, u16 value, u32 mode, u8 x, u8 y, u8 n)
{
    ...
    StringExpandPlaceholders(gStringVar4, gText_xVar1);
    AddTextPrinterParameterized(windowId, FONT_OPTION, gStringVar4, GetStringCenterAlignXOffset(FONT_OPTION, gStringVar4, 48), y, 0, NULL);
}
...
static void ItemStorage_DoItemToss(u8 taskId)
{
    ...
    if (ItemId_GetPocket(gSaveBlock1Ptr->pcItems[pos].itemId) != POCKET_KEY_ITEMS)
    {
