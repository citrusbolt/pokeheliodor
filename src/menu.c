...
void AddTextPrinterForMessage(bool8 allowSkippingDelayWithButtonPress)
{
    ...
    AddTextPrinterParameterized2(0, FONT_OPTION, gStringVar4, GetPlayerTextSpeedDelay(), callback, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}

void AddTextPrinterForMessage_2(bool8 allowSkippingDelayWithButtonPress)
{
    gTextFlags.canABSpeedUpPrint = allowSkippingDelayWithButtonPress;
    AddTextPrinterParameterized2(0, FONT_OPTION, gStringVar4, GetPlayerTextSpeedDelay(), NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}

void AddTextPrinterWithCustomSpeedForMessage(bool8 allowSkippingDelayWithButtonPress, u8 speed)
{
    gTextFlags.canABSpeedUpPrint = allowSkippingDelayWithButtonPress;
    AddTextPrinterParameterized2(0, FONT_OPTION, gStringVar4, speed, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
}
...
{
    LoadMessageBoxAndBorderGfx();
    DisplayMessageAndContinueTask(taskId, 0, DLG_WINDOW_BASE_TILE_NUM, DLG_WINDOW_PALETTE_NUM, FONT_OPTION, GetPlayerTextSpeedDelay(), string, callback);
...
void HofPCTopBar_PrintPair(const u8 *string, const u8 *string2, bool8 noBg, u8 left, bool8 copyToVram)
{
    ...
    if (sHofPCTopBarWindowId != WINDOW_NONE)
    {
        ...
        AddTextPrinterParameterized4(sHofPCTopBarWindowId, FONT_OPTION, 4, 1, 0, 0, color, 0, string);
        if (copyToVram)
            CopyWindowToVram(sHofPCTopBarWindowId, COPYWIN_FULL);
    }
}
...
void PrintMenuActionTextsInUpperLeftCorner(u8 windowId, u8 itemCount, const struct MenuAction *menuActions, const u8 *actionIds)
{
    ...
    printer.fgColor = GetFontAttribute(FONT_OPTION, FONTATTR_COLOR_FOREGROUND);
    printer.bgColor = GetFontAttribute(FONT_OPTION, FONTATTR_COLOR_BACKGROUND);
    printer.shadowColor = GetFontAttribute(FONT_OPTION, FONTATTR_COLOR_SHADOW);
    printer.unk = GetFontAttribute(FONT_OPTION, FONTATTR_UNKNOWN);
    printer.letterSpacing = 0;
    printer.lineSpacing = 0;
    printer.x = 8;
    printer.currentX = 8;
...
static void UNUSED LoadMonIconPalAtOffset(u8 palOffset, u16 speciesId, u8 form)
{
    LoadPalette(GetValidMonIconPalettePtr(speciesId, form), palOffset, PLTT_SIZE_4BPP);
}

static void UNUSED DrawMonIconAtPos(u8 windowId, u16 speciesId, u8 form, u32 personality, u16 x, u16 y)
{
    BlitBitmapToWindow(windowId, GetMonIconPtr(speciesId, form, personality, 1), x, y, 32, 32);
}
    ...
    *(string++) = color + 1;
    ...
    switch (textId)
    {
        ...
        case SAVE_MENU_LOCATION:
            GetMapName(string, gMapHeader.regionMapSectionId, 0);
            break;
