        ...
        if (myLinkPlayerNumber != i)
        {
            switch (GetLinkVersion((u8)gLinkPlayers[i].version, gLinkPlayers[i].versionModifier))
            {
                ...
                case LINK_VERSION_LEAFGREEN:
                    if (gLinkPlayers[i].gender == MALE)
                        linkSpriteId = OBJ_EVENT_GFX_RED;
                    else
                        linkSpriteId = OBJ_EVENT_GFX_LEAF;
                    break;
                ...
                case LINK_VERSION_RECHARGED_YELLOW:
                    if (gLinkPlayers[i].gender == MALE)
                        linkSpriteId = OBJ_EVENT_GFX_RED_RY;
                    else
                        linkSpriteId = OBJ_EVENT_GFX_LEAF_RY;
                    break;
            }
...
{
    return CopyMonFavoritePokeblockName(GetTrueNature(&gPlayerParty[GetLeadMonIndex()]), gStringVar1);
}
...
void ShowDeptStoreElevatorFloorSelect(void)
{
    ...
    xPos = GetStringCenterAlignXOffset(FONT_OPTION, sDeptStoreFloorNames[gSpecialVar_0x8005], 64);
    AddTextPrinterParameterized(sTutorMoveAndElevatorWindowId, FONT_OPTION, sDeptStoreFloorNames[gSpecialVar_0x8005], xPos, 17, TEXT_SKIP_DRAW, NULL);
...
static const u8 *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] =
{
    ...
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_STONE_VENDOR] =
    {
        gText_FireStone3BP,
        gText_WaterStone3BP,
        gText_ThunderStone3BP,
        gText_LeafStone3BP,
        gText_MoonStone3BP,
        gText_SunStone3BP,
		gText_DragonScale3BP,
		gText_MetalCoat3BP,
		gText_UpGrade3BP,
		gText_DeepSeaScale3BP,
		gText_DeepSeaTooth3BP,
        gText_Exit
    },
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR] =
    {
		...
		gText_Stick48BP,
		gText_ThickClub48BP,
		gText_LuckyPunch48BP,
		gText_LaxIncense48BP,
		gText_SeaIncense48BP,
        gText_Exit
    },
...
void SetBattleTowerLinkPlayerGfx(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        switch (GetLinkVersion((u8)gLinkPlayers[i].version, gLinkPlayers[i].versionModifier))
        {
            ...
            default:
                if (gLinkPlayers[i].gender == MALE)
                    VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_BRENDAN_RS);
                else
                    VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_MAY_RS);
                break;
            case LINK_VERSION_FIRERED:
            case LINK_VERSION_LEAFGREEN:
                if (gLinkPlayers[i].gender == MALE)
            ...
            case LINK_VERSION_RECHARGED_YELLOW:
                if (gLinkPlayers[i].gender == MALE)
                    VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_RED_RY);
                else
                    VarSet(VAR_OBJ_GFX_ID_F - i, OBJ_EVENT_GFX_LEAF_RY);
                break;
        }
    }
}
    ...
    if ((menu >= SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1 && menu <= SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR) || menu >= SCROLL_MULTI_COUPON_EXCHANGE)
    {
        ...
        switch (menu)
        {
            ...
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2:
                ...
                break;
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_STONE_VENDOR:
                AddTextPrinterParameterized2(0, FONT_OPTION, sFrontierExchangeCorner_StonesDescriptions[selection], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
                ShowFrontierExchangeCornerItemIcon(sFrontierExchangeCorner_Stones[selection]);
                break;
            case SCROLL_MULTI_BF_EXCHANGE_CORNER_HOLD_ITEM_VENDOR:
                AddTextPrinterParameterized2(0, FONT_OPTION, sFrontierExchangeCorner_HoldItemsDescriptions[selection], 0, NULL, TEXT_COLOR_DARK_GRAY, TEXT_COLOR_WHITE, TEXT_COLOR_LIGHT_GRAY);
