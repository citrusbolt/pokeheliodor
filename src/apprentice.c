        ...
        if (Random() % 2 == 0 || needTMs == TRUE)
        {
            ...
            do
            {
                ...
                do
                {
                    ...
                    shouldUseMove = CanSpeciesLearnTMHM(species, FORM_NONE, id);
                }
                while (!shouldUseMove);
...
static void CreateApprenticeMenu(u8 menu)
{
    ...
    for (i = 0; i < count; i++)
    {
        s32 width = GetStringWidth(FONT_OPTION, strings[i], 0);
        if (width > pixelWidth)
            pixelWidth = width;
    }
    ...
    for (i = 0; i < count; i++)
        AddTextPrinterParameterized(windowId, FONT_OPTION, strings[i], 8, (i * 16) + 1, TEXT_SKIP_DRAW, NULL);
