...
{
    AddTextPrinterParameterized4(windowId, FONT_OPTION, x, y, 0, 0, sTextColors[colorStructId], speed, text);
}
...
static void AddBerryTagTextToBg0(void)
{
    ...
    PrintTextInBerryTagScreen(WIN_BERRY_TAG, gText_BerryTag, GetStringCenterAlignXOffset(FONT_OPTION, gText_BerryTag, 0x40), 1, 0, 1);
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_OPTION, gText_SizeSlash, 0, 1, TEXT_SKIP_DRAW, NULL);
    if (berry->size != 0)
    {
    ...
    {
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_OPTION, gText_ThreeMarks, 0x28, 1, 0, NULL);
    }
}
...
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_OPTION, gText_FirmSlash, 0, 0x11, TEXT_SKIP_DRAW, NULL);
    if (berry->firmness != 0)
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_OPTION, sBerryFirmnessStrings[berry->firmness - 1], 0x28, 0x11, 0, NULL);
    else
        AddTextPrinterParameterized(WIN_SIZE_FIRM, FONT_OPTION, gText_ThreeMarks, 0x28, 0x11, 0, NULL);
}
...
static void PrintBerryDescription1(void)
{
	...
	if (sBerryTag->berryId == ItemIdToBerryType(ITEM_ENIGMA_BERRY))
		AddTextPrinterParameterized(WIN_DESC, FONT_OPTION, gSaveBlock1Ptr->enigmaBerryDesc.description1, 0, 1, 0, NULL);
    else if (gSaveBlock2Ptr->optionsUnitSystem == 1 && sBerryTag->berryId == ItemIdToBerryType(ITEM_WATMEL_BERRY)) // Metric
        AddTextPrinterParameterized(WIN_DESC, FONT_OPTION, sBerryDescriptionPart1_WatmelMetric, 0, 1, 0, NULL);
	else
		AddTextPrinterParameterized(WIN_DESC, FONT_OPTION, berry->description1, 0, 1, 0, NULL);
}
...
static void PrintBerryDescription2(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    if (sBerryTag->berryId == ItemIdToBerryType(ITEM_ENIGMA_BERRY))
		AddTextPrinterParameterized(WIN_DESC, FONT_OPTION, gSaveBlock1Ptr->enigmaBerryDesc.description2, 0, 0x11, 0, NULL);
    else if (gSaveBlock2Ptr->optionsUnitSystem == 1 && sBerryTag->berryId == ItemIdToBerryType(ITEM_WATMEL_BERRY)) // Metric
        AddTextPrinterParameterized(WIN_DESC, FONT_OPTION, sBerryDescriptionPart2_WatmelMetric, 0, 0x11, 0, NULL);
	else
		AddTextPrinterParameterized(WIN_DESC, FONT_OPTION, berry->description2, 0, 0x11, 0, NULL);
}
