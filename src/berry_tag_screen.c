...
static void PrintBerryDescription1(void)
{
	...
	if (sBerryTag->berryId == ItemIdToBerryType(ITEM_ENIGMA_BERRY))
		AddTextPrinterParameterized(WIN_DESC, FONT_NORMAL, gSaveBlock1Ptr->enigmaBerryDesc.description1, 0, 1, 0, NULL);
	else
...
static void PrintBerryDescription2(void)
{
    const struct Berry *berry = GetBerryInfo(sBerryTag->berryId);
    if (sBerryTag->berryId == ItemIdToBerryType(ITEM_ENIGMA_BERRY))
		AddTextPrinterParameterized(WIN_DESC, FONT_NORMAL, gSaveBlock1Ptr->enigmaBerryDesc.description2, 0, 0x11, 0, NULL);
	else
