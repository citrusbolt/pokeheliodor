...
static void Cmd_trytakecaughtmonitem(void);
...
void (* const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_checkcaughtmonhasitem,                   //0xF9
    Cmd_trytakecaughtmonitem                     //0xFA
};
...
static const u8 sTerrainToType[] =
{
    ...
    [BATTLE_TERRAIN_GYM]        = TYPE_NORMAL,
};
...
u8 AI_TypeCalc(u16 move, u16 targetSpecies, u8 targetForm, u8 targetAbility)
{
    s32 i = 0;
    u8 flags = 0;
    u8 type1, type2, moveType;

    if (IsFormValid(targetSpecies, targetForm))
    {
        type1 = gSpeciesInfo[GetFormID(targetSpecies,targetForm)].types[0];
        type2 = gSpeciesInfo[GetFormID(targetSpecies,targetForm)].types[1];
    }
    else
    {
        type1 = gSpeciesInfo[targetSpecies].types[0];
...
static void Cmd_adjustnormaldamage(void)
{
	...
	else if (gBattleMons[gBattlerTarget].hp <= gBattleMoveDamage && gBattleMons[gBattlerTarget].friendship >= 180 && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_TRAINER_HILL | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_SAFARI | BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_EREADER_TRAINER)))
	{
...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            if(IsFormValid(gBattleMons[gBattlerFainted].species, gBattleMons[gBattlerFainted].form))
                calculatedExp = gSpeciesInfo[GetFormID(gBattleMons[gBattlerFainted].species, gBattleMons[gBattlerFainted].form)].expYield * gBattleMons[gBattlerFainted].level / 7;
            else
                calculatedExp = gSpeciesInfo[gBattleMons[gBattlerFainted].species].expYield * gBattleMons[gBattlerFainted].level / 7;

            if (gSaveBlock2Ptr->expAllEnabled && viaExpShare)
            {
                *exp = calculatedExp;
            ...
            else if (gSaveBlock2Ptr->expAllEnabled)
            {
                *exp = calculatedExp;
            ...
            else if (viaExpShare) // at least one mon is getting exp via exp share
            {
                *exp = calculatedExp;
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (!gSaveBlock2Ptr->expAllEnabled && holdEffect != HOLD_EFFECT_EXP_SHARE && !(gBattleStruct->sentInPokes & 1))
            {
            ...
            else
            {
                ...
                if (!gBattleScripting.catchSuccess && !(gBattleTypeFlags & BATTLE_TYPE_TRAINER) && (gBattleMons[0].hp || (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && gBattleMons[2].hp)) && !IsBattlerAlive(GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT)) && !IsBattlerAlive(GetBattlerAtPosition(B_POSITION_OPPONENT_RIGHT)) && !gBattleStruct->wildVictorySong)
...
static void PutMonIconOnLvlUpBanner(void)
{
    ...
    const u8 *iconPtr = GetMonIconPtr(species, form, personality, 1);
...
static void Cmd_various(void)
{
	...
	case VARIOUS_JUMP_IF_ROAR_FAILS:
		if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER))) && GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER && GetBattlerSide(gBattlerTarget) == B_SIDE_OPPONENT && (IsBattlerAlive(gBattlerTarget) && IsBattlerAlive(BATTLE_PARTNER(gBattlerTarget))))
			gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 3);
		else if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER))) && GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT && GetBattlerSide(gBattlerTarget) == B_SIDE_OPPONENT)
			gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 3);
		else
			gBattlescriptCurrInstr += 7;
		return;
...
static void Cmd_forcerandomswitch(void)
{
    ...
    if ((gBattleTypeFlags & BATTLE_TYPE_TRAINER) || ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER))) && GetBattlerSide(gBattlerAttacker) == B_SIDE_OPPONENT && GetBattlerSide(gBattlerTarget) == B_SIDE_PLAYER && (IsBattlerAlive(gBattlerTarget) && IsBattlerAlive(BATTLE_PARTNER(gBattlerTarget)))) || ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_TRAINER))) && GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER && GetBattlerSide(gBattlerTarget) == B_SIDE_PLAYER))
    {
        ...
        for (i = 0; i < PARTY_SIZE; i++)
        {
            ...
            if (species != SPECIES_NONE && species != SPECIES_EGG)
            {
                ...
                if (gBattlerPartyIndexes[gBattlerAttacker] == i)
                    ...
                    ability = GetAbilityBySpecies(species, form, abilityNum);
...
static void Cmd_trydobeatup(void)
{
    ...
    else
    {
        ...
        if (gBattleCommunication[0] < PARTY_SIZE)
        {
            ...
            if (IsFormValid(GetMonData(&party[gBattleCommunication[0]], MON_DATA_SPECIES), GetMonData(&party[gBattleCommunication[0]], MON_DATA_FORM)))
                gBattleMoveDamage = gSpeciesInfo[GetFormID(GetMonData(&party[gBattleCommunication[0]], MON_DATA_SPECIES), GetMonData(&party[gBattleCommunication[0]], MON_DATA_FORM))].baseAttack;
            else
            ...
            if (IsFormValid(gBattleMons[gBattlerTarget].species, gBattleMons[gBattlerTarget].form))
                gBattleMoveDamage /= gSpeciesInfo[GetFormID(gBattleMons[gBattlerTarget].species, gBattleMons[gBattlerTarget].form)].baseDefense;
            else
                ...
                form = GetMonData(&gPlayerParty[i], MON_DATA_FORM);
                    ...
                    if (IsFormValid(species, form))
                    {
                        if (GetMonData(&gPlayerParty[i], MON_DATA_ABILITY_NUM))
                            ability = gSpeciesInfo[GetFormID(species, form)].abilities[1];
                        else
                            ability = gSpeciesInfo[GetFormID(species, form)].abilities[0];
                    }
                    else
                    {
                        if (GetMonData(&gPlayerParty[i], MON_DATA_ABILITY_NUM))
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        else
		{
            ...
            if (IsFormValid(gBattleMons[gBattlerTarget].species, gBattleMons[gBattlerTarget].form))
                catchRate = gSpeciesInfo[GetFormID(gBattleMons[gBattlerTarget].species, gBattleMons[gBattlerTarget].form)].catchRate;
            else
        ...
        switch (gLastUsedItem)
        {
            ...
            case ITEM_NET_BALL:
                ...
                break;
            case ITEM_DIVE_BALL:
                if (GetCurrentMapType() == MAP_TYPE_UNDERWATER || TestPlayerAvatarFlags(PLAYER_AVATAR_FLAG_SURFING) || gFishingEncounter)
                {
                    ballMultiplier = 350;
                }
                else
            ...
            case ITEM_NEST_BALL:
                ...
                {
                    ballMultiplier = (41 - gBattleMons[gBattlerTarget].level) * 10;
            ...
            case ITEM_TIMER_BALL:
                ballMultiplier = gBattleResults.battleTurnCounter * 3 + 100;
                if (ballMultiplier > 400)
                    ballMultiplier = 400;
                break;
            case ITEM_DUSK_BALL:
                if (GetCurrentTimeOfDay() == TIME_NIGHT || gMapHeader.mapType == MAP_TYPE_UNDERGROUND )
                    ballMultiplier = 350;
                else
                    ballMultiplier = 100;
                break;
            ...
            case ITEM_LEVEL_BALL:
                ...
                break;
            case ITEM_LURE_BALL:
                if (gFishingEncounter)
                    ballMultiplier = 500;
                else
                    ballMultiplier = 100;
                break;
            case ITEM_HEAVY_BALL:
                if (GetPokedexHeightWeight(gBattleMons[gBattlerTarget].species, 1) < 1000)
                    heavyBallModifier = -20;
                else if (GetPokedexHeightWeight(gBattleMons[gBattlerTarget].species, 1) < 2000)
                    heavyBallModifier = 0;
                else if (GetPokedexHeightWeight(gBattleMons[gBattlerTarget].species, 1) < 3000)
                    heavyBallModifier = 20;
                else
                    heavyBallModifier = 30;
                ballMultiplier = 100;
                break;
            case ITEM_LOVE_BALL:
                if (gBattleMons[gBattlerAttacker].species == gBattleMons[gBattlerTarget].species)
                {
                    u8 gender1 = GetMonGender(&gPlayerParty[gBattlerPartyIndexes[gBattlerAttacker]]);
                    u8 gender2 = GetMonGender(&gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]]);
            ...
            case ITEM_DREAM_BALL:
                if (gDreamEncounter)
                    ballMultiplier = 2550;
                else if (gBattleMons[gBattlerTarget].status1 & STATUS1_SLEEP)
                    ballMultiplier = 400;
                else
                    ballMultiplier = 100;
                break;
            case ITEM_HISUI_POKE_BALL:
                ballMultiplier = 75;
                break;
            case ITEM_HISUI_GREAT_BALL:
                ballMultiplier = 113;
                break;
            case ITEM_HISUI_ULTRA_BALL:
                ballMultiplier = 150;
                break;
            case ITEM_HISUI_FEATHER_BALL:
                if (IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_FLYING) || gBattleMons[gBattlerTarget].ability == ABILITY_LEVITATE)
                    ballMultiplier = 125;
                else
                    ballMultiplier = 100;
                break;
            case ITEM_HISUI_WING_BALL:
                if (IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_FLYING) || gBattleMons[gBattlerTarget].ability == ABILITY_LEVITATE)
                    ballMultiplier = 175;
                else
                    ballMultiplier = 150;
                break;
            case ITEM_HISUI_JET_BALL:
                if (IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_FLYING) || gBattleMons[gBattlerTarget].ability == ABILITY_LEVITATE)
                    ballMultiplier = 275;
                else
                    ballMultiplier = 200;
                break;
            case ITEM_HISUI_HEAVY_BALL:
                ballMultiplier = 125;
                break;
            case ITEM_HISUI_LEADEN_BALL:
                ballMultiplier = 200;
                break;
            case ITEM_HISUI_GIGATON_BALL:
                ballMultiplier = 275;
                break;
        }
    ...
    case 1:
        if (!gPaletteFade.active)
        {
            ...
            gBattleCommunication[TASK_ID] = DisplayCaughtMonDexPage(SpeciesToNationalPokedexNum(species),
                                                                        gBattleMons[gBattlerTarget].form,
    ...
    case 2:
        if (!gPaletteFade.active)
        {
            ...
            DoNamingScreen(NAMING_SCREEN_CAUGHT_MON, gBattleStruct->caughtMonNick,
                           ...
                           GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]], MON_DATA_FORM),
...
static void Cmd_checkcaughtmonhasitem(void)
{
	u16 item = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]], MON_DATA_HELD_ITEM);
...
static void Cmd_trytakecaughtmonitem(void)
{
    ...
    case 1:
        ...
        break;
    case 2:
		item = GetMonData(&gEnemyParty[gBattlerPartyIndexes[gBattlerTarget]], MON_DATA_HELD_ITEM);
		AddBagItem(item, 1);
		item = ITEM_NONE;
