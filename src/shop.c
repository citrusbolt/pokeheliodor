...
static void BuyMenuPrint(u8 windowId, const u8 *text, u8 x, u8 y, s8 speed, u8 colorSet)
{
    AddTextPrinterParameterized4(windowId, FONT_OPTION, x, y, 0, 0, sShopBuyMenuTextColors[colorSet], speed, text);
}

static void BuyMenuDisplayMessage(u8 taskId, const u8 *text, TaskFunc callback)
{
    DisplayMessageAndContinueTask(taskId, WIN_MESSAGE, 10, 14, FONT_OPTION, GetPlayerTextSpeedDelay(), text, callback);
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    PrintMoneyAmount(WIN_QUANTITY_PRICE, GetMoneyCoordinates(), 1, sShopData->totalCost, TEXT_SKIP_DRAW);
