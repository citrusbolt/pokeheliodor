...
OBJ_DIR_NAME := $(BUILD_DIR)/heliodor
...
PATCH_NAME := $(FILE_NAME).bps
LASTFLASHED_ROM_NAME := $(FILE_NAME)_lastflashed.gba
...
O_LEVEL ?= 2
CPPFLAGS := $(INCLUDE_CPP_ARGS) -Wno-trigraphs -DMODERN=$(MODERN) -DBUILD_REPO_BRANCH=$(BUILD_REPO_BRANCH) -DBUILD_VERSION=$(BUILD_VERSION) -DBUILD_TIME=$(BUILD_TIME) -DBUILD_DIRTY=$(BUILD_DIRTY) -DDEV_BUILD=$(DEV_BUILD)
ifeq ($(MODERN),0)
...
clean: tidy clean-tools clean-generated clean-assets clean-emerald clean-berry-fix
